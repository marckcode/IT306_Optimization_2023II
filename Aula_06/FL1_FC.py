...
m.Vnom  = Param(initialize=11.0 / sqrt(3),  # magnitude da tensão nominal
               mutable=True)
m.V0 = Param(m.ob, m.od, mutable=True)      # estimativa da magnitude de tensão   
m.P0 = Param(m.ol, m.od, mutable=True)      # estimativa do fluxo de potencia ativa
m.Q0 = Param(m.ol, m.od, mutable=True)      # estimativa do fluxo de potencia reativa

# declaração das variáveis
m.Vqdr = Var(m.ob, m.od, domain=Reals, initialize=0.0)   # variável que representa o quadrado de V[i,d]
...
def calculo_magnitude_corrente(m, i, j, d):
    return m.Iqdr[i,j,d] * (m.V0[j,d])**2 == m.P0[i,j,d] * m.P[i,j,d] + m.Q0[i,j,d] * m.Q[i,j,d]
...
# Sequência de problemas de PLs - 3 Iterações
for itr in range(0, 1):
    for i in instance.ob:
        for d in instance.od:
            instance.V0[i, d] = sqrt(instance.Vqdr[i, d].value)
...
print('Objective: %.4f' % instance.custo())
