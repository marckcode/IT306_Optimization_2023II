...
m.obout = Set(m.ob, domain=m.ob, initialize=[])
m.obin = Set(m.ob, domain=m.ob, initialize=[])

def Populate_In_and_Out(m):
    # itera sobre os ramos
    for i, j in m.ol:
        m.obin[j].add(i)
        m.obout[i].add(j)
...
m.Iqdr = Var(m.ol, m.od, initialize=0)       # variável que representa o quadrado de I[i,j,d]  
m.QS = Var(m.ob, m.od, domain=Reals)         # potência reativa fornecida pela subestação no nó i
m.P = Var(m.ol, m.od, domain=Reals)          # fluxo de potência ativa no circuito ij
m.Q = Var(m.ol, m.od, domain=Reals)          # fluxo de potência reativa no circuito ij

# definição da função objetivo
def custo(m):
    return sum(m.cls[d]*m.alpha[d]*sum(m.PS[i,d] for i in m.ob) for d in m.od) 
m.custo = Objective(rule=custo, sense=minimize)

def balance_potencia_activa(m, i, d):
    return sum(m.P[j,i,d] for j in m.obin[i]) - sum(m.P[i,j,d] for j in m.obout[i]) + m.PS[i,d] == m.PD[i,d]
m.balance_potencia_activa = Constraint(m.ob, m.od, rule=balance_potencia_activa)

def balance_potencia_reativa(m, i, d):
    return sum(m.Q[j,i,d] for j in m.obin[i]) - sum(m.Q[i,j,d] for j in m.obout[i]) + m.QS[i,d] == m.QD[i,d]
m.balance_potencia_reativa = Constraint(m.ob, m.od, rule=balance_potencia_reativa)

def queda_magnitude_tensao(m, i, j, d):
    return m.Vqdr[i,d] - 2*(m.R[i,j]*m.P[i,j,d]+m.X[i,j]*m.Q[i,j,d]) - m.Vqdr[j,d] == 0
