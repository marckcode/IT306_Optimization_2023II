...
def custo(m):
    return m.cIGD * m.PGDmax + 365 * sum(m.p[c] * m.delta * m.cOS[t] * m.PS[t, c] for t in m.T for c in m.C) + \
...
def balanco_potencia_ativa(m, t, c):
    return m.PS[t, c] + m.fGD[t] * m.PGDmax == m.PD[t] * (1 - m.xD[t, c])
...
for t in instance.T:
    for c in instance.C:
        res2 += instance.p[c] * instance.delta * instance.cCC * instance.PD[t] *instance.xD[t, c].value
print(f"365*(sum'{'t in T'}' sum'{'c in C'}' p[c]*delta*cCC*PD[t]*xD[t,c])) = {int(365 * res2)}") 
