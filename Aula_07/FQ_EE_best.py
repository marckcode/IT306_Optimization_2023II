...
m.yP  = Var(m.ol, domain=Binary)            # estado do fluxo de potência ativa no circuito ij
m.yQ  = Var(m.ol, domain=Binary)            # estado do fluxo de potência reativa no circuito ij
m.xP  = Var(m.ol, RangeSet(m.Y),            # estado do y-ésimo bloco de P[i,j,d]
            domain=Binary)
m.xQ  = Var(m.ol, RangeSet(m.Y),            # estado do y-ésimo bloco de Q[i,j,d]
            domain=Binary)
...
def limite_PM(m, i, j):
    return  m.PM[i, j] <= m.Vmax*m.Imax[i, j] * m.yP[i, j]
m.limite_PM = Constraint(m.ol, rule=limite_PM)

def limite_Pm(m, i, j):
    return  m.Pm[i, j] <= m.Vmax*m.Imax[i, j] * (1 - m.yP[i, j])
m.limite_Pm = Constraint(m.ol, rule=limite_Pm)
...
def limite_QM(m, i, j):
    return  m.QM[i, j] <= m.Vmax*m.Imax[i, j] * m.yQ[i, j]
m.limite_QM = Constraint(m.ol, rule=limite_QM)

def limite_Qm(m, i, j):
    return  m.Qm[i, j] <= m.Vmax*m.Imax[i, j] * (1 - m.yQ[i, j])
m.limite_Qm = Constraint(m.ol, rule=limite_Qm)

def limite_DeltP_1(m, i, j, y):
    return 0 <= m.DeltP[i, j, y]
m.limite_DeltP_1 = Constraint(m.ol, RangeSet(m.Y), rule=limite_DeltP_1)

def limite_DeltP_2(m, i, j, y):
    return m.DeltP[i, j, y] <= m.DeltS[i, j] * m.Vqdr[j] / m.Vnom ** 2
m.limite_DeltP_2 = Constraint(m.ol, RangeSet(m.Y), rule=limite_DeltP_2)

def limite_DeltP_3(m, i, j, y):
    return m.DeltP[i, j, y+1] <= m.DeltS[i, j] * m.xP[i, j, y] * m.Vmax ** 2 / m.Vnom ** 2
m.limite_DeltP_3 = Constraint(m.ol, RangeSet(m.Y-1), rule=limite_DeltP_3)

def limite_DeltP_4(m, i, j, y):
    return m.DeltP[i, j, y] - m.DeltS[i, j] * m.Vqdr[j] / m.Vnom ** 2 +  \
           m.DeltS[i, j] * (m.Vmax**2/m.Vnom**2) * (1 - m.xP[i, j, y]) >= 0
m.limite_DeltP_4 = Constraint(m.ol, RangeSet(m.Y), rule=limite_DeltP_4)

def limite_DeltP_5(m, i, j, y):
    return m.xP[i, j, y-1] >= m.xP[i, j, y]
m.limite_DeltP_5 = Constraint(m.ol, RangeSet(2, m.Y), rule=limite_DeltP_5)

def limite_DeltQ_1(m, i, j, y):
    return 0 <= m.DeltQ[i, j, y]
m.limite_DeltQ_1 = Constraint(m.ol, RangeSet(m.Y), rule=limite_DeltQ_1)

def limite_DeltQ_2(m, i, j, y):
    return m.DeltQ[i, j, y] <= m.DeltS[i, j] * m.Vqdr[j] / m.Vnom ** 2
m.limite_DeltQ_2 = Constraint(m.ol, RangeSet(m.Y), rule=limite_DeltQ_2)

def limite_DeltQ_3(m, i, j, y):
    return m.DeltQ[i, j, y+1] <= m.DeltS[i, j] * m.xQ[i, j, y] * m.Vmax ** 2 / m.Vnom ** 2
m.limite_DeltQ_3 = Constraint(m.ol, RangeSet(m.Y-1), rule=limite_DeltQ_3)

def limite_DeltQ_4(m, i, j, y):
    return (m.DeltQ[i, j, y] - m.DeltS[i, j] * m.Vqdr[j] / m.Vnom ** 2 +  
           m.DeltS[i, j] * (m.Vmax**2/m.Vnom**2) * (1 - m.xQ[i, j, y]) >= 0)
m.limite_DeltQ_4 = Constraint(m.ol, RangeSet(m.Y), rule=limite_DeltQ_4)

def limite_DeltQ_5(m, i, j, y):
    return m.xQ[i, j, y-1] >= m.xQ[i, j, y]
m.limite_DeltQ_5 = Constraint(m.ol, RangeSet(2, m.Y), rule=limite_DeltQ_5)
...
# results = solver_manager.solve(inst, solver="cplex")
print('Minimos Quadrados: %.4f' % inst.minimos_quadrados()) # O erro melhora
